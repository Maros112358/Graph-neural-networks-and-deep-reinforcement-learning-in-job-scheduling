...
\par
From Table 4.1 and Figures 4.1, 4.2, 4.3, and 4.4, we observed that priority dispatching rule \textbf{LRPT} and model \textbf{fjsp-drl} produced similar results as the model \textbf{IEEE-ICCE-RL-JSP}, which had the lowest mean gap. Therefore, we tested the null hypothesis that \textbf{LRPT}, \textbf{fjsp-drl}, and \textbf{IEEE-ICCE-RL-JSP} are from the same population. The p-value for small instances is $45.7\%$, for medium instances $1.1\%$, $3.8\%$ for large instances, and 0.1$\%$ for all instances. We, therefore, reject the null hypothesis that these three models are from the same population for medium and large instances.
For each pair of these three models, we then tested the null hypothesis, which was that the pair of models was equal. Obtained p-values are shown in Table 4.3.

\begin{table}[H]
    Table 4.3: p-values for Kruskal-Wallis test between pairs of models\\
    \vspace{1mm}
    \begin{tabular}{llr}
    \toprule
...
We could reject the null hypothesis only for three pairs from the Kruskal-Wallis test.

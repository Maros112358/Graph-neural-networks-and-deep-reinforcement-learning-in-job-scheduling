...
\par
\textit{Action} $a_t \in \mathcal{A}$ is an eligible operation at time step $t$. Since there is at most one eligible operation from each job, the size of action space is $\left|\mathcal{J}\right|$ \cite{zhang2020learning}.
\par

...
def generate_arrival_times_for_jssp(instance, load_factor, seed):
    ...
    average_time_between_arrivals = np.mean(processor_times) / load_factor
...
def get_dynamic_jssp(instance, arrival_times):
    ...
    '''
    _, M, processor_times, orders_of_machines = parse_instance_taillard(instance)
...
if __name__ == '__main__':
    ...
    # run the experiment
    count = 0
    total = len(LOAD_FACTORS) * len(SEEDS) * len(MODELS) * len(INSTANCES)
    for model in MODELS:
            ...
            for seed in SEEDS:
                for instance in INSTANCES:
                    ...
                    if not filtered_df.empty:
                        ...
                        arrival_times = generate_arrival_times_for_jssp(instance, load_factor=load_factor, seed=seed)
                        dynamic_instance = get_dynamic_jssp(instance, arrival_times)
                        solution, runtime = solve_instance_taillard(dynamic_instance, agent)

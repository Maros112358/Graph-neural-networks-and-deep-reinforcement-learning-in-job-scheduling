...
def test_pdr(pdr: str, instance_path, arrival_times: list | None = None):
    # load env
    env = JSP_Env(args)
    env.load_instance(instance_path, arrival_times)
    _ = env.get_graph_data(args.device)

    # run the model
    i = 0
    while True:
        assert pdr in env.rules, "PDR not in env.rules"
        _, _, done, _, _ = env.step(pdr)
        i += 1
        if done:
            break
...
if __name__ == '__main__':
    PDRS = ["EDD", "FIFO", "LPT", "LOR", "LS", "MOR", "LRPT", "SPT", "SRPT"]
    BENCHMARKS_PATH = "../../../benchmarks/jssp/ta_instances/Taillard_specification/"
    DATA_FILE = 'experiment_static_jssp_pdrs.csv'
    PREV_DATA_FILE = 'experiment_static_jssp_pdrs_save.csv'
    ...
    with open(DATA_FILE, 'w') as f:
        f.write("seed,pdr,instance,makespan,runtime\n")
    ...
    # run experiments
    count = 0
    total = len(seeds) * len(PDRS) * len(instances)
    for pdr in PDRS:
        for seed in seeds:
            for instance in instances:
                ...
                filtered_df = df[(df['seed'] == seed) & (df['pdr'] == pdr) & (df['instance'] == instance_name)]
                if not filtered_df.empty:
                    ...
                    # run experiment
                    start = time.time()
                    makespan = test_pdr(pdr, standard_instance)
                    runtime = time.time() - start

                with open(DATA_FILE, 'a') as f:
                    f.write(f"{seed},{pdr},{instance_name},{makespan},{runtime}\n")

                count += 1
                logging.info(f"{count=} {total=} {seed=} {pdr=} {instance_name=} {makespan=} {runtime=} ")

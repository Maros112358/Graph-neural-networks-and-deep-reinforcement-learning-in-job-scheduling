...
for filename in os.listdir(directory):
    if filename.startswith("experiment_dynamic_jssp_") and filename.endswith(".csv"):
        print(filename)
        ...
        if 'model' in df.columns:
            ...
            df['model'] = filename.removeprefix("experiment_dynamic_jssp_").removesuffix('.csv')
...
# final_updated_df['gap'] = (final_updated_df['makespan'] - final_updated_df['best_makespan']) / (final_updated_df['best_makespan'])

# calculate category
final_updated_df.rename(columns={'model': 'parameter_set'}, inplace=True)
final_updated_df['category'] = final_updated_df['jobs'].apply(categorize)
final_updated_df.to_csv('djsp_preprocess_data.csv', index=False)
